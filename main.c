...
#define DEVICE_NAME                     "LA1"                               /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME               "actnova"                      /**< Manufacturer. Will be passed to Device Information Service. */
...
static ble_jawbone_t                    m_jawbone;
...
static void battery_level_update(void)
{
    ...
    uint8_t  battery_level;
		static uint8_t test[] = {0x1,0x02,0x03,0x04,0x05};
        ...
        )
    {
		...
		TriggerEventData_Send(&m_jawbone,test );
}
...
static void services_init(void)
{
    ...
    ble_bas_init_t   bas_init;
		ble_jawbone_init_t   jawbone_init;
    ...
    APP_ERROR_CHECK(err_code);
		...
		memset(&jawbone_init, 0, sizeof(ble_jawbone_init_t));
		jawbone_init.evt_handler          = NULL;
    jawbone_init.support_notification = true;
    jawbone_init.p_report_ref         = NULL;

    err_code = ble_jawbone_init(&m_jawbone, &jawbone_init);
    APP_ERROR_CHECK(err_code);
...
static void ble_evt_dispatch(ble_evt_t * p_ble_evt)
{
    ...
    ble_bas_on_ble_evt(&m_bas, p_ble_evt);
	  ble_jawbone_on_ble_evt(&m_jawbone, p_ble_evt);
